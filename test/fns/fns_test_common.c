...
void test_common_assert_parse_failure(CuTest* tc, mat_world_t* w, const char* fmt, ...) {
	...
	CuAssertIntEquals(tc, MAT_ARG_COUNT_MISMATCH, mat_err_get_id(mat_world_get_error_info(w)));
...
void test_common_assert_value(CuTest* tc, mat_world_t* w, double ex, const char* fmt, ...) {
	...
	mpq_t t;
	mpq_init(t);
	mat_error_t err = mat_op_calc_value(w, expr, t);
...
mat_error_t check_expr_error(CuTest* tc, mat_world_t* w, const char* fmt, ...) {
	...
	mpq_t r;
	mpq_init(r);
	mat_error_t err = mat_op_calc_value(w, expr, r);
	mpq_clear(r);
	mat_expr_free(expr);
	return err;
