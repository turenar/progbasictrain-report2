...
int main() {
	const char* expr = "Plus [ Times [ Sin [ \n13.4 ] , 3 ] , 2 ]";
	printf("expression:\t%s\n", expr);

	mat_tokenizer_t* tokenizer = mat_tokenizer_new(expr);
	while (true) {
		mat_tokenizer_token_type_t t = mat_tokenizer_next(tokenizer);
		printf("%s:\t[%2zu,%2zu] %s\n", mat_tokenizer_get_token_type_name(t), mat_tokenizer_get_row(tokenizer),
		       mat_tokenizer_get_col(tokenizer), mat_tokenizer_get_token(tokenizer));
